...
import { getCurrentGoPath, getTempFilePath } from './util';
import { GoDocumentSymbolProvider } from './goOutline';
import { getGoConfig } from './config';
import { getTestFlags, goTest, GoTestOutput } from './testUtils';

const profileCPU = 'Profile CPU';
const profileMem = 'Profile Memory';
let profileCount = 0;
const lastProfile = new WeakMap<TestItem, string[]>();
...
export class TestExplorer {
	...
	constructor(
		...
		ctrl.createRunProfile('go test', TestRunProfileKind.Run, (rq) => runTest(this, rq).catch(errored), true);
	}
	...
	async profile(item: TestItem) {
		const profile = await vscode.window.showQuickPick([profileCPU, profileMem, 'Both'], {
			title: 'Profile CPU or memory?'
		});
		if (!profile) return;

		if (profile === 'Both') await runTest(this, { include: [item] }, new Set([profileCPU, profileMem]));
		else await runTest(this, { include: [item] }, new Set([profile]));

		await Promise.all(
			lastProfile.get(item).map(async (path) => {
				const uri = Uri.from({ scheme: 'go-pprof', path });
...
async function runTest(expl: TestExplorer, request: TestRunRequest, profile: Set<string> = new Set<string>()) {
	...
	for (const [dir, items] of collected.entries()) {
		...
		if (profile.size > 0) {
			...
			if (profile.has(profileCPU)) {
				const name = `profile-${id}.cpu.prof`;
				flags.push('--cpuprofile', getTempFilePath(name));
				profiles.push(name);
			}
			if (profile.has(profileMem)) {
				const name = `profile-${id}.mem.prof`;
				flags.push('--memprofile', getTempFilePath(name));
				profiles.push(name);
			}
		}
